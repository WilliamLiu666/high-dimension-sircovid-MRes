...
create_Rn2par_new <- function(pars,scale){
  ...
  pars$par2Rn <- function(x) (log((x - pars_min) / (pars_max - x)) )*scale
  pars$Rn2par <- function(x) (pars_min + pars_max * exp(x/scale)) / (1 + exp(x/scale))
...
HMC_parallel <- function (RnPosterior, gradient_LP_parallel, epsilon, L, current_q, filter,filter2, pars, M, invM,method = 'f1'){
  q = current_q
